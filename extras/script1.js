const products = [
    ...
    function renderProducts() {
      ...
      filtered.forEach(prod => {
        let stars = '★'.repeat(prod.rating) + '☆'.repeat(5 - prod.rating);
        list.innerHTML += `
          <div class="col-md-3">
            <div class="product-card text-center">
              ...
              <button class="btn btn-primary btn-sm" onclick="addToCart(${prod.id})">Add to Cart</button>
            </div>
          </div>
        `;
      });
    ...
    function renderCart() {
      ...
      for (let id in cart) {
        ...
        count += qty;
        cartEl.innerHTML += `<div class="d-flex justify-content-between align-items-center mb-2">
          <span>${item.name}</span>
          <div>
